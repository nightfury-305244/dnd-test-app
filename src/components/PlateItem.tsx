...
const PlateItem = ({ item, moveItem, textOnPlate, dateOnPlate }: {item: DraggableItem, moveItem: MoveItemFunction, textOnPlate: string, dateOnPlate: string}) => {
    ...
    return (
        <div ref={ref} style={{ position: 'absolute', left: item.position.x, top: item.position.y, cursor: 'move'  }}>
            <img src={item.url} alt={`Dropped ${item.type}`} style={{ width: '100px', height: '100px' }} />
            <div className="plate-text" style={{ position: 'absolute', width: '100%', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center' }}>
                {textOnPlate}<br/>
                {dateOnPlate}
            </div>
        </div>
